 ...
 * 결과가 어느 문항 세트에서 나왔는지.
 *
 * - `'base'` 기본 45문항. 유형당 5문항(다섯 facet 각 1문항) → 점수 5~25.
 * - `'full'` 전체 90문항. 유형당 10문항 → 점수 10~50.
 *
 * 둘은 점수 척도도 신뢰도도 다르므로 **같은 것으로 취급하지 않는다.** 이 값이
 * `lib/code.ts`의 버전 바이트를 고르고, 결과 화면이 어떤 한계를 적을지를 정한다.
 */
export type ResultKind = 'base' | 'full';

/**
 * 9유형 원점수. 범위는 문항 세트가 정한다 — 유형당 `n`문항 × 리커트 1~5이므로
 * 기본 검사(n=5)는 **5~25**, 전체 90문항(n=10)은 **10~50**이다.
