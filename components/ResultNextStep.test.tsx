...
describe('ResultNextStep', () => {
  ...
  it('공유 링크 방문자에게는 이어하기 대신 새 검사를 안내한다', () => {
    ...
    expect(screen.getByRole('link', { name: '45문항 검사 시작하기' }).getAttribute('href')).toBe('/test');
    expect(screen.queryByText('남은 45문항')).toBeNull();
  });

  it('이 세션에서 결과를 만든 사람에게만 남은 45문항 이어하기를 보여준다', async () => {
    window.sessionStorage.setItem(LAST_RESULT_KEY, 'owned-code');
    render(<ResultNextStep code="owned-code" />);

    await waitFor(() => expect(screen.getByText('남은 45문항')).toBeTruthy());
    expect(screen.getByRole('link', { name: '45문항 더 답하고 정확도 높이기' }).getAttribute('href')).toBe(
      '/test?continue',
    );
  });
});
